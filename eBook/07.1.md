...
2.引用类型：变量存储的值是一个内存地址(所以实质上是指针)，这个内存地址对应的空间才是真正存储数据（值），内存通常在堆上分配（也有可能会被分配在栈上），当没有任何变量引用这个地址时，改地址对应的数据空间就成为一个垃圾，由GC来回收。
```
...
new(T)和&T{}是等效的：
arrx := &[5]int{} //{}作用：初始化
fmt.Println(arrx) //&[0 0 0 0 0]
arrz := &[5]int{5, 5, 5, 5, 5} //{}作用：初始化
fmt.Println(arrz) //&[5 5 5 5 5]
arry := new([5]int) //new关键字直接初始化为零值
fmt.Println(arry) //&[0 0 0 0 0]
```
验证：这种通过new(T)或等价的&T{}生成的指向数组的指针类型的变量存的值是数组第一个元素的内存地址，即指针指向的是数组第一个元素，这其实在我们意料之中，无论是C++还是Java，都是利用数组的第一个元素
```
arr1 := [5]int{0, 1, 2, 3, 4}
arr2 := &arr1
fmt.Println(&arr1[0], &arr1[1], &arr1[2], &arr1[3], &arr1[4]) //0xc00000a3c0 0xc00000a3c8 0xc00000a3d0 0xc00000a3d8 0xc00000a3e0

//打印指针类型变量存的值
fmt.Println(unsafe.Pointer(arr2))                             //0xc00000a3c0
```
```go
拓展小知识：
Golang 的数组指针和指针数组
...
而指针数组pfArr则是一个数组内的元素全是指针类型
一下是打印出来的结果
&[0 0 0 0 0 2]
[0xc04204a080 0xc04204a088]
}
```
