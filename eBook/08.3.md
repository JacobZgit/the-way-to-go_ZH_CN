...
输出结果：
    ...
    或者
    ...（其他随机结果）
但是输出map1``` fmt.Println(map1)```，结果始终都是固定的
```
    map[1:1 2:2 3:3 4:4]
```
...
> 
> 即多次遍历map的结果可能是不同的。(Map没有index的概念，只有 K-V 键值对！！！)
> 
> 最后，注意别混淆了，对于任意一个map来说，它的底层数据结构都是**确定的**，都是**固定好的**。只是多次对map进行**遍历**的结果可能是不同的。
